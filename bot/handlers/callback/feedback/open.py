...
async def conversation_open(
    ...
    async with IntercomClient(INTERCOM_TOKEN) as intercom:
        ...
        last_message = next((i for i in sorted(comments, key=lambda x: x.get('created_at', 0), reverse=True)), None)
            ...
            buttons.append(
                types.InlineKeyboardButton(
                    ...
                    callback_data=flow_cb.new(
                        id=1, action="page", conv_id=conversation_id,
                    ),
                )
            )
